...
export function manageWelcomeMessage(settingsManager: ISettingsManager): void {
  const themeName = getConfig().inspect(constants.vscode.iconThemeSetting)
    .globalValue;

  if (
    !settingsManager.getState().welcomeShown &&
    themeName !== constants.extensionName
  ) {
    showWelcomeMessage();
    return;
  }
  ...
  if (
    ...
    !getConfig().vsicons.dontShowNewVersionMessage
  ) {
    ...
    vscode.window
      ...
      .then(
        btn => {
          ...
          if (
            btn.title ===
            i18nManager.getMessage(LangResourceKeys.aboutOfficialApi)
          ) {
          ...
          if (
            btn.title === i18nManager.getMessage(LangResourceKeys.seeReadme)
          ) {
...
function showNewVersionMessage(): void {
  vscode.window
    .showInformationMessage(
      `${i18nManager.getMessage(LangResourceKeys.newVersion)} v${
        extensionSettings.version
      }`,
    ...
    .then(
      btn => {
        ...
        if (
          btn.title === i18nManager.getMessage(LangResourceKeys.seeReleaseNotes)
        ) {
          open(constants.urlReleaseNote);
        } else if (
          btn.title === i18nManager.getMessage(LangResourceKeys.dontShowThis)
        ) {
