  ...
  context('ensures that', function() {
    context('the extension', function() {
      ...
      it('detects an Angular project from dependencies', function() {
        ...
        expect(
          pad.getInfo(packageJson, models.Projects.angular),
        ).to.not.be.null;
      });
      ...
      it('gets the project info version correctly', function() {
        ...
        const projectInfo: models.IProjectInfo = pad.getInfo(
          packageJson,
      ...
      context('gets from search result', function() {
        ...
        it('the project info if it matches', function() {
          const ver1: any = '{ "dependencies": { "@angular/core": "0.0.0" } }';
          sandbox
            .stub(fs, 'readFileSync')
            .returns(ver1);
          const results = [{ fsPath: '' }] as models.IVSCodeUri[];
          const projectInfo: models.IProjectInfo = pad.getProjectInfo(
            results,
            models.Projects.angular,
          );
          expect(projectInfo).to.not.be.null;
        ...
        it('no project info if it does not match', function() {
          const ver2: any = '{ "dependencies": { "meteor": "0.0.0" } }';
          sandbox
            .stub(fs, 'readFileSync')
            .returns(ver2);
          const results = [{ fsPath: '' }] as models.IVSCodeUri[];
      ...
      context('detects that special folder icons are', function() {
        ...
        it('enabled', function() {
          const iconManifest: any = '{ "iconDefinitions": { "_fd_aws": {} } }';
          sandbox.stub(fs, 'readFileSync').returns(iconManifest);
          expect(mr.iconsDisabled('aws', false)).to.be.false;
        });

        it('disabled', function() {
          const iconManifest: any = '{ "iconDefinitions": { "_fd_git": {} } }';
          sandbox.stub(fs, 'readFileSync').returns(iconManifest);
      ...
      context('detect a project when detection is enabled', function() {
        ...
        it(`and has detected a 'package.json' file`, function() {
          ...
          const findFiles = sinon
            .stub()
            .resolves([{ fsPath: path }] as models.IVSCodeUri[]);
          userConfig.projectDetection.disableDetect = false;
          return pad.detectProject(findFiles, userConfig).then(res => {
            expect(res)
              .to.be.an('array')
      ...
      context('Angular icons are disabled and the workspace is', function() {
        context('an Angular project and', function() {
          it('Angular preset is true', function() {
            ...
            const i18nManager = sinon.createStubInstance(
              LanguageResourceManager,
            );
            const res = pad.checkForAngularProject(
              preset,
              ngIconsDisabled,
              isNgProject,
              i18nManager,
