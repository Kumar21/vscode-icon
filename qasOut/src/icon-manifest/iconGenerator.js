...
class IconGenerator {
    constructor(vscode, defaultSchema, configCustomIconFolderPath, avoidCustomDetection = false) {
        ...
        this.manifestFolderPath = path.join(__dirname, '../../../', 'qasOut/src');
    ...
    getCustomIconFolderPath(folderPath) {
        if (!folderPath) {
            return this.settings.vscodeAppUserPath;
        }
        const fPath = folderPath.trim();
        if (path.isAbsolute(fPath) ||
            !this.settings.workspacePath ||
            !this.settings.workspacePath.length) {
            return fPath;
        }
    ...
    buildFolders(folders, hasDefaultLightFolder) {
        ...
        return _.sortBy(folders.supported.filter(x => !x.disabled && x.icon), item => item.icon).reduce((old, current) => {
            const defs = old.defs;
    ...
    buildFiles(files, hasDefaultLightFile) {
        const sts = this.settings.extensionSettings;
        return _.sortedUniq(_.sortBy(files.supported.filter(x => !x.disabled && x.icon), item => item.icon)).reduce((old, current) => {
            const defs = old.defs;
            const names = old.names;
            ...
            if (hasGlobDefinitions) {
                utils
                    .combine(current.filenamesGlob, current.extensionsGlob)
                    .forEach(populateFn);
            }
            return old;
        }, {
    ...
    buildJsonStructure(files, folders, schema) {
        ...
        schema.iconDefinitions = Object.assign(Object.assign(Object.assign({}, schema.iconDefinitions), res.folders.defs), res.files.defs);
