...
const i18nManager = new languageResourceManager_1.LanguageResourceManager(vscode.env.language);
function manageWelcomeMessage(settingsManager) {
    const themeName = vscode_extensions_1.getConfig().inspect(constants_1.constants.vscode.iconThemeSetting)
        .globalValue;
    if (!settingsManager.getState().welcomeShown &&
        themeName !== constants_1.constants.extensionName) {
        showWelcomeMessage();
        return;
    }
    if (settingsManager.isNewVersion() &&
        !vscode_extensions_1.getConfig().vsicons.dontShowNewVersionMessage) {
...
function showWelcomeMessage() {
    const displayMessage = () => {
        vscode.window
            .showInformationMessage(i18nManager.getMessage(models_1.LangResourceKeys.welcome), { title: i18nManager.getMessage(models_1.LangResourceKeys.activate) }, { title: i18nManager.getMessage(models_1.LangResourceKeys.aboutOfficialApi) }, { title: i18nManager.getMessage(models_1.LangResourceKeys.seeReadme) })
            .then(btn => {
            if (!btn) {
                return;
            }
            ...
            if (btn.title ===
                i18nManager.getMessage(models_1.LangResourceKeys.aboutOfficialApi)) {
...
function showNewVersionMessage() {
    vscode.window
        .showInformationMessage(`${i18nManager.getMessage(models_1.LangResourceKeys.newVersion)} v${extensionSettings_1.extensionSettings.version}`, { title: i18nManager.getMessage(models_1.LangResourceKeys.seeReleaseNotes) }, { title: i18nManager.getMessage(models_1.LangResourceKeys.dontShowThis) })
        .then(btn => {
        if (!btn) {
            return;
        }
